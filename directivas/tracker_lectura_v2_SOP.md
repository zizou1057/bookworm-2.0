...
- *Fechas Timestamp*: Las columnas de tipo `timestamp with time zone` (como `end_date`) en Supabase devuelven las fechas con hora en formato ISO (`YYYY-MM-DDTHH...Z`). Para que los `<input type="date">` de HTML las rendericen correctamente, es obligatorio hacerles `.split('T')[0]` al momento de cargar los datos desde la BD, de lo contrario el campo de fecha aparecerá vacío en la UI.
